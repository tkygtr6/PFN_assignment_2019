...
cd ../server
DATA_DIR=../test/data_test3 FLASK_APP=server.py flask run 1>/dev/null 2>/dev/null &
sleep 2
...
echo "test3_3"
diff <(MAXTIME=40 CAPACITY=10 PRIORITY_RANGE=2 NAIVE=1 ./worker 2>/dev/null | awk '{print $2}') <(cat ../test/ans/test3_3 | awk '{print $2}') 

echo "test3_4"
diff <(MAXTIME=40 CAPACITY=10 PRIORITY_RANGE=2 ./worker 2>/dev/null | awk '{print $2}') <(cat ../test/ans/test3_4 | awk '{print $2}') 
